...
import { updateGearAction } from "@/actions/gear.action";
...
export interface GearItemData extends GearFormValues {
  id: string;
}
...
export function EditGearModal({ gear, categories, trigger }: EditGearModalProps) {
  ...
  const form = useForm<GearFormValues>({
    ...
    defaultValues: {
      ...
      brand: gear.brand || "",
      images: gear.images || [],
      features: gear.features?.length ? gear.features : [""],
  ...
  // Re-sync form values if `gear` prop updates dynamically
  useEffect(() => {
    if (gear) {
      form.reset({
        name: gear.name,
  ...
  const onSubmit: SubmitHandler<GearFormValues> = async (values) => {
    setLoading(true);
    try {
      const res = await updateGearAction(gear.id, values);
    ...
    <Dialog open={isOpen} onOpenChange={setOpen}>
        ...
        <DialogTrigger
          render={
            <Button type="button" variant="outline" size="sm">
              <Edit className="mr-2 h-4 w-4" /> Edit Gear
            </Button>
          }
        />
      )}
      ...
      <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update pricing, specifications, availability, or images for this item.
          </DialogDescription>
        </DialogHeader>
